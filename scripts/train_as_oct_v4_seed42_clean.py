"""Train combined v4 AS-OCT-only seed42 baseline with formal output names.

This launcher intentionally trains only the AS-OCT image baseline on the
already prepared seed42 split manifest. It reuses the existing AS-OCT training
helpers, but writes the combined v4 artifacts directly into the requested
report/checkpoint directories.
"""
...
DEFAULT_REPORT_DIR = "artifacts/reports/combined_batch_01_02_03_04/as_oct_only_baseline_seed42"
DEFAULT_CHECKPOINT_DIR = "artifacts/checkpoints/combined_batch_01_02_03_04/as_oct_only_seed42"
MEASUREMENT_ONLY_RF_MAE_UM = 169.44


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train combined v4 AS-OCT-only seed42 clean 200-epoch baseline."
    )
    ...
    parser.add_argument("--measurement_only_rf_mae", type=float, default=MEASUREMENT_ONLY_RF_MAE_UM)
...
def write_summary(
    ...
    lines: List[str] = [
        "# Combined v4 AS-OCT-only seed42 baseline",
        "",
        "## Training selection",
...
def main() -> None:
    ...
    expected_counts = {"train": 241, "val": 51, "test": 51}
    ...
    log_path = report_dir / "as_oct_v4_seed42_training_log.csv"
    overall_metrics_path = report_dir / "as_oct_v4_seed42_overall_metrics.csv"
    range_metrics_path = report_dir / "as_oct_v4_seed42_range_metrics.csv"
    predictions_path = report_dir / "as_oct_v4_seed42_predictions.csv"
    summary_path = report_dir / "as_oct_v4_seed42_summary.md"
    if log_path.exists():
        log_path.unlink()
    ...
    if len(test_predictions) != 51:
        raise ValueError(f"Expected 51 test predictions, got {len(test_predictions)}")
