"""Run intake QC for real_export_batch_03 without building training manifests."""
...
RAW_ROOT = PROJECT_ROOT / "data/raw/real_export_batch_03"
OUTPUT_DIR = PROJECT_ROOT / "artifacts/reports/real_export_batch_03/intake_qc"
QC_REFERENCE_DATE = date(2026, 6, 8)
...
def load_prior_patient_uids() -> set[str]:
    uids: set[str] = set()
    for path in [
        ...
        if "patient_uid" in df.columns:
            uids.update(df["patient_uid"].dropna().astype(str).str.lower())
    return uids


def collect_records(patient_dirs: list[Path], patient_root: Path) -> list[ImageRecord]:
...
def build_visit_inventory(records: list[ImageRecord]) -> pd.DataFrame:
    ...
    for record in records:
        if record.modality not in {"oct_raw", "oct_2d_analysis"}:
            continue
...
def build_patient_inventory(
        ...
        rows.append(
            {
                ...
                "duplicates_batch01_batch02_patient_uid": patient_uid in prior_uids,
                "has_OD": "OD" in eyes,
                "has_OS": "OS" in eyes,
    ...
    columns = [
        ...
        "naming_issue",
        "duplicates_batch01_batch02_patient_uid",
        "has_OD",
        "has_OS",
        "eyes_detected",
        "num_eyes_detected",
...
def build_missing_report(patient_df: pd.DataFrame, visit_df: pd.DataFrame, records: list[ImageRecord]) -> pd.DataFrame:
    ...
    for row in patient_df.to_dict(orient="records"):
        ...
        if bool(row["duplicates_batch01_batch02_patient_uid"]):
            add_issue(rows, patient_uid, folder, "", "duplicate_patient_uid", "patient_uid exists in batch_01/batch_02 summary")
...
def write_summary(patient_df: pd.DataFrame, visit_df: pd.DataFrame, missing_df: pd.DataFrame, path: Path) -> None:
    ...
    manual_df = missing_df[
        missing_df["issue_type"].isin(
            ["naming_anomaly", "duplicate_patient_uid", "missing_eye", "missing_preop_as_oct", "missing_pod1_2d_analysis", "future_exam_date", "unparsed_oct_filename"]
        )
    ] if not missing_df.empty else missing_df
    manual_targets = (
    ...
    can_extract = recognized_patients == 50 and not pod1_eye_keys.empty and int(pod1_eye_keys.shape[0]) > 0

    lines = [
        "# Batch 03 接入 QC 总结",
        "",
        "本次 QC 只检查 `data/raw/real_export_batch_03/` 接入状态；未训练模型，未合并 combined manifest，未修改 batch_01 / batch_02 结果。",
        "",
        "## 识别结果",
        "",
        f"- patient 文件夹总数：{folder_count}",
        f"- 识别到的去标识化 patient_uid 数：{recognized_patients}",
        f"- 严格符合 `patient_数字` 命名的文件夹数：{strict_patients}",
        f"- 实际识别到眼数：{len(eye_keys)}",
        f"- 具备术前 AS-OCT raw 的眼数：{len(preop_eye_keys)}",
        f"- 具备 POD1 label 候选（第一术后日期 2DAnalysis）的眼数：{len(pod1_eye_keys)}",
        f"- 与 batch_01 / batch_02 重复的 patient_uid 数：{int(patient_df['duplicates_batch01_batch02_patient_uid'].sum()) if not patient_df.empty else 0}",
        "",
        "## 主要 QC 发现",
        "",
        ...
        "",
        (
            "- batch_03 可以进入下一步 2DAnalysis measurement extraction；建议先处理/备注上述人工检查项，尤其是非严格命名目录。"
            if can_extract
            else "- batch_03 暂不建议直接进入 extraction；请先处理 patient 数、眼别或 POD1 候选缺失问题。"
        ),
        "",
        "## 输出文件",
        "",
        "- `artifacts/reports/real_export_batch_03/intake_qc/batch03_patient_inventory.csv`",
        "- `artifacts/reports/real_export_batch_03/intake_qc/batch03_eye_visit_inventory.csv`",
        "- `artifacts/reports/real_export_batch_03/intake_qc/batch03_missing_data_report.csv`",
        "- `artifacts/reports/real_export_batch_03/intake_qc/batch03_intake_qc_summary.md`",
        "",
        "## Patient ID 列表",
        "",
        ", ".join(patient_df["patient_uid"].astype(str).tolist()) if not patient_df.empty else "",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    prior_uids = load_prior_patient_uids()

    records = collect_records(patient_dirs=patient_dirs, patient_root=patient_root)
    visit_df = build_visit_inventory(records)
    patient_df = build_patient_inventory(
        patient_dirs=patient_dirs,
        records=records,
        visit_df=visit_df,
        prior_uids=prior_uids,
    )
    missing_df = build_missing_report(patient_df=patient_df, visit_df=visit_df, records=records)

    patient_df.to_csv(OUTPUT_DIR / "batch03_patient_inventory.csv", index=False, encoding="utf-8")
    visit_df.to_csv(OUTPUT_DIR / "batch03_eye_visit_inventory.csv", index=False, encoding="utf-8")
    missing_df.to_csv(OUTPUT_DIR / "batch03_missing_data_report.csv", index=False, encoding="utf-8")
    write_summary(patient_df, visit_df, missing_df, OUTPUT_DIR / "batch03_intake_qc_summary.md")
