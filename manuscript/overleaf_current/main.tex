...
\subsection{Exploratory Implanted-ICL Variable Pilot}

A separate exploratory pilot evaluated whether implanted-ICL variables added predictive information beyond AS-OCT and the five preoperative measurements. This cohort comprised 26 eyes from 14 patients (12 bilateral and 2 unilateral) and was not pooled with the main v5 cohorts. The implanted-ICL input contained size, power, and model (ICL3). Seven prespecified Ridge-based combinations of AS-OCT, measurements, and ICL3 were assessed using five repeats of five-fold patient-grouped nested cross-validation. AS-OCT inputs used a frozen 512-dimensional ResNet18 representation. Scaling, PCA, categorical encoding, hyperparameter selection, and Ridge fitting were restricted to the relevant training partitions. The prespecified primary contrast compared AS-OCT plus measurements plus ICL3 with AS-OCT plus measurements. Delta MAE was defined as primary minus comparator, so negative values favored the primary model. Uncertainty was summarized using the frozen 10,000-resample patient-clustered percentile bootstrap. This analysis was exploratory and not confirmatory; full methods and results are provided in the Supplementary Material.
...
In the separate 26-eye pilot, AS-OCT-only had the numerically lowest mean MAE among the seven frozen models ($153.268 \pm 10.057~\mu$m). The prespecified AS-OCT-plus-measurement comparator achieved $162.059 \pm 16.008~\mu$m, and the primary model adding ICL3 achieved $160.859 \pm 18.568~\mu$m. The mean repeat-level delta was $-1.200~\mu$m, while the patient-clustered bootstrap point estimate was $-0.972~\mu$m with a 95\% percentile interval from $-10.941$ to $8.847~\mu$m (Fig.~\ref{fig:pilot_primary_delta}). Seven patients improved, none tied, and seven worsened. Thus, adding implanted ICL size, power, and model did not show stable incremental predictive value; the interval crossing zero does not establish equivalence or absence of value. The cohort contained 18 medium-vault and 8 high-vault eyes, no low-vault eyes, and sparse high-vault coverage in several folds. All rankings and stratum results are therefore descriptive; complete tables and additional figures are reported in the Supplementary Material.

\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{figures/fig_pilot_primary_paired_delta.png}
\caption{Exploratory implanted-ICL variable pilot primary comparison. Delta MAE is primary minus comparator, so negative values favor adding implanted ICL size, power, and model to AS-OCT plus measurements. Repeat-level deltas and the frozen patient-clustered bootstrap 95\% percentile interval are shown. The interval crosses zero; the analysis is exploratory and not confirmatory.}
\label{fig:pilot_primary_delta}
\end{figure*}

\subsection{Summary of Main Findings}

The repeated full-cohort baselines showed competitive Measurement RF and AS-OCT unimodal predictors, while simple concat fusion did not provide stable benefit. In the strict matched cohort, leakage-safe validation-tuned additive fusion was a modest conventional baseline with split-dependent gains but no robust overall improvement over matched unimodal references. Oracle Best-of-Two demonstrated substantial retrospective per-eye complementarity, yet G0/G1/G2 diagnostics did not show that the available inference-time signals could reliably learn which modality to trust. In a separate exploratory 26-eye pilot, adding implanted ICL size, power, and model likewise did not show stable incremental predictive value. High-vault error and prediction-range compression remained unresolved.
...
\subsection{Interpretation of the Exploratory Implanted-ICL Variable Pilot}

The implanted-ICL pilot provided a deliberately limited test of whether three readily available device variables could address information gaps suggested by the main analysis. Adding ICL3 to AS-OCT plus measurements reduced mean MAE by only about $1.2~\mu$m, with a clustered interval spanning both favorable and unfavorable effects and an even 7/7 patient split. This pattern does not support stable incremental value in the present sample. Redundancy with anatomical information, measurement noise, nonlinear interactions not captured by the prespecified Ridge models, and small-sample instability are plausible but unproven explanations. The numerically lowest pilot MAE for AS-OCT-only is directionally consistent with the main finding that routine fusion did not provide stable gains, but estimates from the separate cohorts must not be pooled or compared inferentially. Larger prospectively specified cohorts, adequate low- and high-vault representation, prespecified uncertainty procedures, and external validation are needed before confirmatory claims about implanted-ICL variables can be made.
...
Several limitations remain. The study is retrospective, single-project, and modest in size, with no external validation. Repeated splits reduce dependence on one partition but do not replace independent validation, and repeated-test pooled eye-level diagnostics may contain the same physical eye in multiple outer test splits. Low- and high-vault eyes are relatively sparse, limiting tail-specific inference. The formal main v5 predictors used only two preoperative modality families: AS-OCT images and five CASIA2 2DAnalysis measurements; device- and procedure-related variables were not available as formal inputs in that cohort. A separate 26-eye pilot explored implanted ICL size, power, and model without stable incremental evidence, but did not evaluate orientation or other surgical factors and was too small for confirmatory or stratum-specific inference. Future studies should assess whether incorporating richer variables improves modality trust estimation and tail performance. Finally, the reliability features used in G2 were prediction-dispersion proxies rather than calibrated uncertainty estimates, and the NO-GO result applies to the evaluated strategies, current inputs, and current cohort rather than to every possible multimodal model.
